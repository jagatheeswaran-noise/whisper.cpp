...
class SlotExtractor {
    ...
    private fun extractUnit(text: String): String? {
        ...
        when {
            ...
            walkingMovementRegex.containsMatchIn(text) -> return "count"  // Default to count for walking/movement
    ...
    private fun extractFeature(text: String): String? {
        val features = mapOf(
            ...
            "raise to wake" to "\\b(?:raise\\s+(?:to|2|too)\\s+(?:wake|bake)|race\\s+(?:to|2|too)\\s+wake|lift\\s+(?:to|2|too)\\s+wake|tap\\s+(?:to|2|too)\\s+wake|double\\s+tap\\s+(?:to|2|too)\\s+wake|touch\\s+(?:to|2|too)\\s+wake|tilt\\s+(?:to|2|too)\\s+wake|pick\\s+up\\s+(?:to|2|too)\\s+wake|wrist\\s+raise|raise\\s+wrist|lift\\s+wrist|wake\\s+on\\s+(?:raise|lift|tap|touch)|gesture\\s+wake|motion\\s+wake|wake\\s+gesture|screen\\s+wake|auto\\s+wake|smart\\s+wake)\\b",
            ...
            "autoHR" to "\\b(?:auto\\s+hr|autoHR|autohr|auto-hr|auto-HR|auto(?:,?\\s+|,\\s*)hr|auto(?:,?\\s+|,\\s*)HR|auto\\s+heart\\s+rate|auto-\\s*heart\\s+rate|auto(?:,?\\s+|,\\s*)heart\\s+rate|automatic\\s+heart\\s+rate|auto\\s+heart|continuous\\s+heart\\s+rate|continuous\\s+hr|always\\s+on\\s+heart\\s+rate|heart\\s+rate\\s+monitoring|hr\\s+monitoring|continuous\\s+heart\\s+monitoring|background\\s+heart\\s+rate|24\\s+7\\s+heart\\s+rate|heart\\s+rate\\s+tracking)\\b",
    ...
    private fun extractState(text: String): String? {
        return when {
            // OFF state - checked first to prioritize explicit disable/off commands
